...
def build_processed_file():
    ...
    total_crime_count = (
        all_data.groupby(["Canton", "Distrito"]).size().reset_index(name="Crimen total desde 2021")
    )

    years_total = one_total.merge(two_total, on=["Canton", "Distrito"], how="outer") \
    ...
    # keep only fields needed in the map
    merged = merged[
        [
            "NOM_DIST",
